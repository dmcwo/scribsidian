...
${quotesArray.map((q, i) => `${i + 1}. "${q.text}"`).join('\n\n')}`
      ...
      const authorLastName = parsedSource.author.split(',')[0].trim(); // For citations
      const authorFullName = parsedSource.author.includes(',') 
        ? parsedSource.author.split(',').reverse().map(p => p.trim()).join(' ')
        : parsedSource.author; // Full name for filename
...
`
  ...
  const downloadAsZip = async () => {
    // Note: This requires JSZip which isn't available in Claude artifacts
    // This function will only work when running locally
    alert('ZIP download requires running this tool locally. Use: npx run-claude-artifact your-file.tsx');
  };
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-900 font-serif mb-2">
                    <strong>💡 Running Locally? Try Direct Downloads:</strong>
                  </p>
                  ...
                  <p className="text-xs text-blue-700 italic">
                    These buttons work when running locally: <code className="bg-blue-100 px-1">npx run-claude-artifact your-file.tsx</code>
                  </p>
                </div>
                
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                  <p className="text-sm text-amber-900 font-serif mb-2">
                    <strong>In Claude.ai? Use "View & Copy Notes"</strong>
                  </p>
                  <p className="text-xs text-amber-800">
                    Copy each markdown file individually and paste into your Obsidian vault. This preserves proper formatting.
                  </p>
                </div>
                
                <p className="text-xs text-stone-500 italic text-center bg-stone-50 p-3 rounded">
                  💡 Tip: To run this tool locally with direct file downloads, copy the artifact code and use: <code className="bg-stone-200 px-1">npx run-claude-artifact your-file.tsx</code>
                </p>
